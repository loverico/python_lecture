...
def convert_examples_to_features(examples, seq_length, tokenizer, ignore_index=0):
    ...
    for example in examples:
        ...
        logger.info(f"subwords is {len(subwords)}")
...
def _make_dataloader(tokens, max_seq_length, batch_size, tokenizer, eval_mode=False):
    examples = []
    for i,t in enumerate(tokens):
            examples.append(InputExample(unique_id=i, text=t))
